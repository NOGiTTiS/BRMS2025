        ...
        <form action="<?php echo URLROOT; ?>/user/register" method="post" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="first_name" id="first_name" 
                           class="w-full px-4 py-2 text-gray-700 bg-white/80 border <?php echo (!empty($data['first_name_err'])) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                           value="<?php echo htmlspecialchars($data['first_name']); ?>" required>
                    <span class="text-red-300 text-sm font-semibold"><?php echo $data['first_name_err']; ?></span>
                </div>
                <div>
                    <label for="last_name" class="block mb-2 text-sm font-medium text-white">นามสกุล</label>
                    <input type="text" name="last_name" id="last_name" 
                           class="w-full px-4 py-2 text-gray-700 bg-white/80 border <?php echo (!empty($data['last_name_err'])) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                           value="<?php echo htmlspecialchars($data['last_name']); ?>">
                    <span class="text-red-300 text-sm font-semibold"><?php echo $data['last_name_err']; ?></span>
                </div>
            </div>
            <div>
                <label for="username" class="block mb-2 text-sm font-medium text-white">ชื่อผู้ใช้งาน</label>
                <input type="text" name="username" id="username" 
                       class="w-full px-4 py-2 text-gray-700 bg-white/80 border <?php echo (!empty($data['username_err'])) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                       value="<?php echo htmlspecialchars($data['username']); ?>" required>
                <span class="text-red-300 text-sm font-semibold"><?php echo $data['username_err']; ?></span>
            </div>
            <div>
                <label for="email" class="block mb-2 text-sm font-medium text-white">อีเมล</label>
                <input type="email" name="email" id="email" 
                       class="w-full px-4 py-2 text-gray-700 bg-white/80 border <?php echo (!empty($data['email_err'])) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                       value="<?php echo htmlspecialchars($data['email']); ?>" required>
                <span class="text-red-300 text-sm font-semibold"><?php echo $data['email_err']; ?></span>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="password" name="confirm_password" id="confirm_password"
                       class="w-full px-4 py-2 text-gray-700 bg-white/80 border <?php echo (!empty($data['confirm_password_err'])) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                       value="" required>
                <span class="text-red-300 text-sm font-semibold"><?php echo $data['confirm_password_err']; ?></span>
            </div>
            <div>
